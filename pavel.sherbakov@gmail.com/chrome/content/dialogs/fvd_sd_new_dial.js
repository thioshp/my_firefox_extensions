...
var speedDialBuilder = opener.fvd_speed_dial_Builder;
...
function setPreviewImage( url ){
  ...
  if( url ){
    ...
    if( url == "thumb_failed" ){
      speedDialBuilder._setFailImageForSnippet( previewBox, 200 );
    }
    else{
      speedDialBuilder._setPreviewImageForSnippet( previewBox, url, 200, document.getElementById( "preview_type" ).value );
    }
...
window.addEventListener("load", function _loadListener() {
  ...
  document.getElementById("pick-local-file").addEventListener("command", function() {
    var nsIFilePicker = Components.interfaces.nsIFilePicker;
    var fp = Components.classes["@mozilla.org/filepicker;1"].createInstance(nsIFilePicker);
    ...
    if (res != nsIFilePicker.returnCancel){
      var thefile = fp.file;
      var fileUri = fp.fileURL.spec;
      document.getElementById("url").value = fileUri;
    }
  });
