...
var HtmlSearch = {
  ...
  displayCanTurnOfNewTabPopup: function(){
    ...
    if( settings.getBoolVal( "sd.display_can_turn_off_newtab" ) ) {
      var popup = document.getElementById("canTurnOfNewTabPopup");
      var top = window.innerHeight/2 - popup.offsetHeight/2;
      var left = document.body.clientWidth/2 - popup.offsetWidth/2;
  ...
  displayCantArrangeInPopularGroup: function(){
    var settings = this.settings();
    this.showOptions( "cantArrangeInPopularGroup", null, null, {
      top: 20,
      left: document.body.clientWidth/2 - document.getElementById("cantArrangeInPopularGroup").offsetWidth/2
    } );
  ...
  localizeElem: function(el) {
    ...
    while(iterator.hasMoreElements()){
      ...
      var e = el.querySelectorAll( ".loc_"+elem.key );
  ...
  setTheme: function(theme) {
    var oldThemeLink = document.getElementById("themeSheet");
    if(oldThemeLink) {
      oldThemeLink.parentNode.removeChild(oldThemeLink);
    }
    var themeLink = document.createElement("link");
    themeLink.setAttribute("rel", "stylesheet");
    themeLink.setAttribute("id", "themeSheet");
    themeLink.setAttribute("href", "chrome://fvd.speeddial/skin/sd/skin/dials/themes/" + theme + "/search.css");
...
HtmlSearch.Templates = {
  _cache: {},
  get: function(name, options, cb) {
    options = options || {};
    function _ret(res) {
      if(options.fragment) {
    ...
    if(this._cache[name]) {
      return _ret(this._cache[name]);
    }
    var xhr = new XMLHttpRequest();
    xhr.open("GET", "chrome://fvd.speeddial/content/templates/" + name);
    xhr.onload = function() {
      _ret(xhr.responseText);
    };
    xhr.onerror = cb;
    xhr.send(null);
  }
