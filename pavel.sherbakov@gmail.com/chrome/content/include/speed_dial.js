...
var fvd_sd_EventEmitter =
  Components.utils.import("resource://fvd.speeddial.modules/eventemitter.js").EventEmitter;
...
var fvd_speed_dial_speedDialSSD = {
  ...
  // this value is manually calculated and should be recalculated when top right menu width
  // is changed, or list button/plus button in groups main list are changed
  GROUPS_MAIN_LIST_MENU_TAKEN_SPACE: 470,
  GROUPS_MAIN_LIST_TAKEN_SPACE_MENU_HIDDEN: 120,
  ...
  MAX_GROUPS_LIMIT: 100, // max 100 groups
  FVD_SD_BG_FILE_NAME: fvd_speed_dial_Config.BG_FILE_NAME,
  ...
  */
  ...
  listenFor: [
    ...
    'FVD.Toolbar-SD-Dial-Title-Updated',
  ...
  onBuildCompleted: new fvd_sd_EventEmitter(),
  ...
  _getRecentClosedTabs: function( count ){
    ...
    for( var i = 0; i != closedTabs.length; i++ ){
      ...
      else if( entry.children && entry.children[0] && entry.children[0].title ){
...
//        continue;
  ...
  _displayMode: function(){
    ...
    if( mode == "last_selected" ){
      try{
        ...
        if(this.displayModesList.indexOf(mode) == -1) {
          throw "not allowed mode";
        }
      }
      catch( ex ){
        mode = "top_sites";
  ...
  _refereshMostVisitedThumb: function( url ){
    ...
    this.clearImageCacheMostVisited(url, true);
  ...
  _displayNewDialWindow: function( inputData, isModify, disableItems ){
    ...
    openDialog(
      'chrome://fvd.speeddial/content/dialogs/fvd_sd_new_dial.xul',
      '',
      'chrome,titlebar,toolbar,modal,centerscreen',params
    );
  ...
  swapDials: function( fromId, toId ){
    ...
    fvd_speed_dial_Builder._setAnimStyles(fromElem, true);
    fvd_speed_dial_Builder._setAnimStyles(toElem, true);
    ...
    (function(from, to, fromId, toId){
      setTimeout( function(){
        ...
        fvd_speed_dial_Builder._setAnimStyles( cloneTo );
        fvd_speed_dial_Builder._setAnimStyles( cloneFrom );

        cloneTo.style.opacity = "";
        cloneFrom.style.opacity = "";

        fvd_speed_dial_Builder._setAnimStyles(cloneTo, false);
        fvd_speed_dial_Builder._setAnimStyles(cloneFrom, false);
        ...
        fvd_speed_dial_Builder._assignCellEvents( cloneFrom, fromDial );
        fvd_speed_dial_Builder._assignCellEvents( cloneTo, toDial );
  ...
  // get font size/family and margins measurements for main list groups elements
  // it is used for measuring width of groups before placing it in main list
  // or in additional list
  _getMainListGroupStyles: function() {
    var groupsContainer = document.querySelector("#groups_list");
    var groupElem = document.querySelector("#groups_list .group");
    var removeGroupElem = false;
    if(!groupElem) {
      removeGroupElem = true;
      groupElem = document.createElement("hbox");
      groupElem.className = "group";
      groupElem.style.visibility = "hidden";
      groupsContainer.appendChild(groupElem);
    }
    var computedStyle = window.getComputedStyle(groupElem);
    var paddingLeft = 0;
    var paddingRight = 0;
    if(computedStyle.paddingLeft && typeof computedStyle.paddingLeft === "string") {
      paddingLeft = parseFloat(computedStyle.paddingLeft.replace("px", "")) || 0;
    }
    if(computedStyle.paddingRight && typeof computedStyle.paddingRight === "string") {
      paddingRight = parseFloat(computedStyle.paddingRight.replace("px", "")) || 0;
    }
    var resultStyles = {
  ...
  refreshGroupsList: function( params ) {
    ...
    var groupsBoxMaxWidth = 0;
    if(fvd_speed_dial_gFVDSSDSettings.getBoolVal("sd.main_menu_displayed")) {
      groupsBoxMaxWidth = parent.window.innerWidth - this.GROUPS_MAIN_LIST_MENU_TAKEN_SPACE;
    }
    else {
      groupsBoxMaxWidth = parent.window.innerWidth - this.GROUPS_MAIN_LIST_TAKEN_SPACE_MENU_HIDDEN;
    }

    var groupsActiveWidth = 0;
    var maxGroupsInMainList = 0;

    var groupStyles = this._getMainListGroupStyles();

    if( fvd_speed_dial_gFVDSSDSettings.getBoolVal("sd.enable_popular_group") ) {
      var popularGroupText = this.getPopularGroupFullTitle();
      var popularGroupTextMeasuredWidth = fvd_speed_dial_Misc.measureTextWidth(
        popularGroupText, groupStyles["font-size"], groupStyles["font-family"]
      );
      groupsActiveWidth +=
        groupStyles["padding-left"] + groupStyles["padding-right"] +
        popularGroupTextMeasuredWidth + this.GROUP_MARGIN;
    }

    if( this.canRestorePreviousSession() ) {
      var restorePreviousSessionWidth =
        document.getElementById("restoreSessionContainer").getElementsByClassName("group")[0].boxObject.width;
      groupsActiveWidth += this.GROUP_MARGIN + restorePreviousSessionWidth;
    }

    for( var i = 0; i != groups.length; i++ ) {
      var groupText = groups[i].name + " ("+this.storage.count(groups[i].id)+")";
      var groupTextMeasuredWidth = fvd_speed_dial_Misc.measureTextWidth(
        groupText, groupStyles["font-size"], groupStyles["font-family"]
      );
      var groupSize = groupTextMeasuredWidth +
        groupStyles["padding-left"] + groupStyles["padding-right"];
      if(groupSize > this.GROUP_WIDTH) {
        groupSize = this.GROUP_WIDTH;
      }
      groupSize += this.GROUP_MARGIN
  ...
  // get popular pseudo group title with number of elements
  getPopularGroupFullTitle: function() {
  ...
  refreshGroupsListAllGroupsLabel: function(){
    var title = this.getPopularGroupFullTitle();
    ...
    if( listChangedDials != false ){
      for( var i = 0; i != listChangedDials.length; i++ ){
        try{
          ...
          fvd_speed_dial_Builder._setAnimStyles(elem, true);
  ...
  refreshCollapsedMessage: function() {
    ...
    if(!this.expandState()) {
      if(expandMessage.hasAttribute("hidden")) {
        LightCollapsedMessage.show();
      }
    }
    else {
  ...
  rebuildCellById: function( id ){
    ...
    try{
      ...
      var cell = fvd_speed_dial_Builder.dial( dial );
  ...
  buildCells: function(params) {
    ...
    if(this._displayMode() == "top_sites") {
      var dials = params.dials || this.storage.listDials(
        false,
        this._getGroupId(),
      ...
      if(this._displayModeType() == "thumbs") {
        ...
        for(var i = 0; i != dials.length; i++) {
          ...
          if(dials[i] == "plus") {
            if(this._displayPlusCells()) {
              dial = fvd_speed_dial_Builder.plusCell();
            }
          }
          else if(dials[i] == "empty_plus") {
            if(this._displayPlusCells()) {
              dial = fvd_speed_dial_Builder.plusCell(true, i);
            }
          }
          else {
            dial = fvd_speed_dial_Builder.dial(dials[i], params);
          }
        ...
        if(
          this._getGroupId() == "all" &&
          dials.length >= fvd_speed_dial_gFVDSSDSettings.getIntVal("sd.all_groups_limit_dials")
        ) {
          countPlusCells = 1;
        }
        else{
        ...
        if(this._displayPlusCells()) {
          for(var i = 0; i < countPlusCells; i++) {
            var dial = fvd_speed_dial_Builder.plusCell();
            row.appendChild( dial );
          }
        }
        ...
        for (var i = 0; i != dials.length; i++) {
          if(dials[i] == "plus") {
           continue; // no plus cell fol dials in list type
            ...
            dial = fvd_speed_dial_Builder.dial( dials[i] );
          }
      ...
      if(this._displayModeType() == "thumbs") {
        ...
        for (var i = 0; i != data.length; i++) {
          ...
          var cell = fvd_speed_dial_Builder.mostVisitedCell( data[i] );
        ...
        var column = null;
        for (var i = 0; i != data.length; i++) {
          var dial = fvd_speed_dial_Builder.mostVisitedCell( data[i] );
      ...
      var column = null;
        ...
        var dial = fvd_speed_dial_Builder._buildListElem( tabs[i] );
    ...
    this.onBuildCompleted.callListeners(params);
  },
  ...
  clearImageCacheMostVisited: function(url, remove) {
    try{
      var file = this._previewFileById( this._md5(url) );
      if(remove) {
        file.remove(true);
      }
      var url = FVDSSDMisc.fileToNativeUri(file);
  ...
  refreshPopupedDial: function(){
    ...
    fvd_speed_dial_ScreenController.refreshSpeedDialTitle(this.popupedDial);
  },
    ...
    try{
      ...
      if( dial ) {
        ...
        if (params.ok) {
          ...
          if(updateData.url != dial.url) {
            // user changed url
            if(!updateData.hand_changed) {
              // title for dial is auto-generated, so need to reset the title
              updateData.title = "";
            }
          }
  ...
  adaptFrameSize: function( resizeFrameToMin ){
    ...
    if(this.scrollingMode() != "horizontal") {
  ...
  scrollingMode: function(){
    return fvd_speed_dial_gFVDSSDSettings.getStringVal( "sd.scrolling" );
  },

  setScrollingMode: function( mode ){

    fvd_speed_dial_gFVDSSDSettings.setStringVal( "sd.scrolling", mode );
...
//          dump( "rows: " + rows + ", dial height: "+dialHeight+", height: " + rows * dialHeight + "("+wHeightPerspective+")\n" );
  ...
  refreshAllDials: function(){
    if( this._displayMode() == "top_sites" ){
      ...
      for( var i = 0; i != dials.length; i++ ){
        ...
        fvd_speed_dial_ScreenController.refreshSpeedDialTitle(dials[i].id);
      }
    }
    else if( this._displayMode() == "most_visited" ){
      var data = this._getMostVisitedTabs( fvd_speed_dial_gFVDSSDSettings.getIntVal("sd.max_most_visited_records"), this._mostVisitedInterval(), this._mostVisitedGroupBy() );
      for( var i = 0; i != data.length; i++ ){
        ...
        this.clearImageCacheMostVisited(data[i].url, true);
  ...
  _tabSelectListener: function( tab ){
    ...
    if(browser.contentWindow == window.parent) {
      ...
      // refresh only if speeddial is not showing search results now
      if(!document.querySelector("window").hasAttribute("dial-search-show-results")) {
    ...
    displayIfNeed: function() {
      ...
      AddonManager.getAddonByID( fvd_speed_dial_gFVDSSD.selfId(), function(addon){
        ...
        try{
          ...
          if( now - installTime >= fvd_speed_dial_Config.SECONDS_USAGE_FOR_RATE_MESSAGE ){
            that.display();
          }
        }
        catch( ex ){
  ...
  HasSyncUpdatesMessage: {
    ...
    manualSync: function(){
      ...
      if( !fvd_speed_dial_Sync.fvdSynchronizerAvailable() ){
        ...
        fvd_speed_dial_gFVDSSDSettings.displayWindow("paneSdSync");
        return;
      }
  ...
  _display: function(){
    ...
    fvd_speed_dial_Themes.setForDocument(
      document, fvd_speed_dial_gFVDSSDSettings.getStringVal("sd.active_theme")
    );
    ...
    if(typeof fvd_speed_dial_sd_prefObserverInst !== "undefined") {
  ...
  Search: {
    ...
    doSearch: function(searchText) {
      ...
      searchText = searchText || document.getElementById("q").value.trim();
      if(searchText) {
        parent.document.location =
          "http://fvdmedia.com/addon_search/?q="+encodeURIComponent(searchText)+"&from="+key;
      }
    },
  ...
  init: function(){
    ...
    // unhighlight search bar after 3 seconds
    setTimeout(function() {
      self.Search.unHighlight();
    }, 3000);
  },
...
var fvd_speed_dial_observerStruct = {
    ...
    switch (aTopic) {
      ...
      case 'FVD.Toolbar-SD-Dial-Title-Updated':
