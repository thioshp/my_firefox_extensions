...
Components.utils.import("resource://fvd.speeddial.modules/migrations.js");
Components.utils.import("resource://fvd.speeddial.modules/poweroffidle.js");
var {fvd_speed_dial_Themes} = Components.utils.import("resource://fvd.speeddial.modules/themes.js", {});
...
fvd_speed_dial_FVD_SSD = function(){
  ...
  function isCurrentWindowPrivate() {
  ...
  this.refreshURLBarButton = function(){
    ...
    if(!gBrowser.currentURI || gBrowser.currentURI.spec === SPEED_DIAL_URL) {
      show = false;
    }
    if(show) {
  ...
  this.observerStruct =  {
      ...
      case "FVD.Toolbar-SD-Dial-Page-Closed":
        ...
        self.clearFramesByDial(aData);
        self.closedDialsIds.push( parseInt(aData) );
  ...
  this.init = function() {
      ...
      AddonManager.getAddonByID(FVD_TOOLBAR_ID, function(addon){
        ...
        self.isFirstStart( function( firstStart, isFirstRun, currentVersion ){
          if( firstStart ){
            ...
            //url = "http://nimbus.everhelper.me/welcome_page/welcome-dial-firefox-2.html";
            ...
            if( isFirstRun ) {
              ...
              // force set white theme on first run
              // check for user image is empty, to not overwrite user background is first run detected
              // incorectly
              if(fvd_speed_dial_gFVDSSDSettings.getStringVal("sd.bg.url") === "") {
                fvd_speed_dial_Themes.set("dark");
              }
              // set speed dial as home page
              fvd_speed_dial_gFVDSSDSettings.setSdAsHomePage(true);
            }
          }
          else{
    ...
    gBrowser.tabContainer.addEventListener("TabAttrModified", function(event) {
      ...
      if(gBrowser.tabContainer.selectedItem == tab) {
        self.refreshURLBarButton();
      }
    }, false);
  };
