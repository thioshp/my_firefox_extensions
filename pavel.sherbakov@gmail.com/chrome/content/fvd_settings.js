...
speedDialSSD = {
  ...
  BG_FILE_NAME: fvd_speed_dial_Config.BG_FILE_NAME
};
...
function FVD_Settings()
{
  ...
  this.resetDialsClicks = function() {
    var r = promptService.confirm( window,
            fvd_speed_dial_FVDSSDToolbarProperties.getString( "fvd.toolbar",  "confirm.reset_clicks.title"),
            fvd_speed_dial_FVDSSDToolbarProperties.getString( "fvd.toolbar",  "confirm.reset_clicks.text"));
    if(r) {
      storage.resetDialsClicks();
      observer.notifyObservers(null, 'FVD.Toolbar-SD-Dial-Shedule-Rebuild', true);
      promptService.alert( window,
                fvd_speed_dial_FVDSSDToolbarProperties.getString( "fvd.toolbar",  "alert.success.reset_clicks.title"),
                fvd_speed_dial_FVDSSDToolbarProperties.getString( "fvd.toolbar",  "alert.success.reset_clicks.text"));
    }

  };

  this.refreshLocalBgFileExistanceError = function() {
    var localFile = Components.classes["@mozilla.org/file/local;1"]
      .createInstance(Components.interfaces.nsIFile);
    var container = document.querySelector("#bgLocalFileBox");
    var isError = true;
    try {
      ...
      if(isError) {
        container.setAttribute("error", 1);
      }
      else {
  ...
  this.refreshPreview = function( getCurrent ){
    ...
    var previewElem = document.querySelector(".bgPreview vbox");
  ...
  this.init = function()
  {
    ...
    document.querySelector("#suggestUseSync button").addEventListener("click", function() {
      goToTab("paneSdSync");
    }, false);
    ...
    document.getElementById("bg_parallax_depth").value = fvd_speed_dial_gFVDSSDSettings.getIntVal("sd.bg.parallax.depth");
    ...
    document.getElementById("fvd_sd_bg_parallax_depth").addEventListener( "change", function(){
      self.refreshPreview();
    }, false );
    ...
    var previewElem = document.getElementsByClassName( "bgPreview" )[0].getElementsByTagName("vbox")[0];
    var bgRatio = window.screen.width / window.screen.height
    var bgHeight = previewElem.boxObject.width/bgRatio;
    previewElem.style.minHeight = bgHeight + "px";
    previewElem.style.maxHeight = bgHeight + "px";
    ...
    this.bgAdjuster.elem = previewElem;
    ...
    if( window.arguments && window.arguments[0].evaluate ){
    ...
    if( fvd_speed_dial_Sync.fvdSynchronizerAvailable() ){
      ...
      document.getElementById("suggestUseSync").removeAttribute("hidden");
    }
    ...
    document.getElementById("btn-turnoff-autoupdate").addEventListener("click", function() {
      var r = promptService.confirm( window,
              fvd_speed_dial_FVDSSDToolbarProperties.getString( "fvd.toolbar",
                "sd.batch_turn_off_autoupdate_confirm.title"),
              fvd_speed_dial_FVDSSDToolbarProperties.getString( "fvd.toolbar",
                "sd.batch_turn_off_autoupdate_confirm.text"));
      if(r) {
        var dialsChanged = storage.turnOffAutoUpdateBatch();
        dialsChanged.forEach(function(global_id) {
          fvd_speed_dial_Sync.syncData(["dials"], global_id);
        });
        promptService.alert( window,
          fvd_speed_dial_FVDSSDToolbarProperties.getString("fvd.toolbar", "sd.dials_preview_updated_success.title"),
          fvd_speed_dial_FVDSSDToolbarProperties.getString("fvd.toolbar", "sd.dials_preview_updated_success.text") );
      }
    }, false);

    document.getElementById("btn-set-autoupdate").addEventListener("click", function() {
      var args = {};
      openDialog('chrome://fvd.speeddial/content/dialogs/set_autoupdate.xul', '',
        'chrome,titlebar,toolbar,centerscreen,modal', args);
      if(args.interval) {
        var dialsChanged = storage.setAutoUpdateBatch(args.interval);
        dialsChanged.forEach(function(global_id) {
          fvd_speed_dial_Sync.syncData(["dials"], global_id);
        });
        promptService.alert( window,
    ...
    document.getElementById("fvd_sd_bg_file_path").addEventListener("change", function() {
      self.refreshLocalBgFileExistanceError();
    }, false);
  };
  ...
  this.pickScreenLocalFile = function() {
    ...
    if (res == nsIFilePicker.returnOK){
      ...
      this.refreshLocalBgFileExistanceError();
    }
  }
  ...
  this.backgroundTypeChange = function(){
    ...
    document.getElementById( "parallax_effect_depth_container" ).setAttribute( "hidden", type != "parallax" );
    document.getElementById( "backgroundImageType" ).setAttribute( "hidden", type == "no_image" );
  }
  ...
  this.reportBug = function(){
    var url = "http://fvdmedia.com/to/s/speeddial_bug/";
