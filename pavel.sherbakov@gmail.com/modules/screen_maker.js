...
var {_dbg} = Components.utils.import("resource://fvd.speeddial.modules/dbg.js", {});
try {
...
var _fvd_speed_dial_ScreenMaker = function(){
    ...
    try {
      Downloads.fetch(url, file).then(callback, callback);
    }
    catch(ex) {
	...
	var imageProcessor = function( screenData, imageProcessor_callback ){
      ...
      _dbg("Start process image " + screenData.imageUrl);
        ...
        function(cCallback) {
          ...
          if(!screenData.getTitle || !screenData.url) {
            return cCallback();
          }
          screenData.noLoadImages = true;
          screenData.onlyTitle = true;
          _dbg("Need to get title for dial: " + screenData.url);
  ...
  // Frame processor
	...
	var frameProcessor = function(screenData, frameProcessor_callback) {
      ...
      onStateChange: function(aWebProgress, aRequest, aStateFlags, aStatus){
        ...
        if (aStatus !== 0 /*Components.results.NS_ERROR_UNKNOWN_HOST*/) {
          var goodStatuses = [Components.interfaces.nsISocketTransport.STATUS_RESOLVING,
            ...
            2153578530 // not documented and not found in sources(wtf), appears when use tracking protection
          ];
        ...
        // disable alert messages on page
        var w = iframe.contentWindow.wrappedJSObject;
        w.alert = function() {};
        w.confirm = function() {};
        w.prompt = function() {};
			},
      ...
      QueryInterface: function(aIID){
        ...
        var contentType = iframe.contentDocument.contentType;
        ...
        _dbg("processing url done " + screenData.url);
				frameProcessor_callback({
					title: getTitleOfFrame(),
					canvas: canvas,
					result: true,
		...
		this.process = function() {
      _dbg("Start process url: " + screenData.url);

      if(screenData.onlyTitle) {
        try {
          // check if this local file
          var _uri = Components.classes["@mozilla.org/network/io-service;1"]
                      .getService(Components.interfaces.nsIIOService).newURI(screenData.url, null, null);
          if(_uri.scheme.toLowerCase() == "file") {
            var _url = _uri.QueryInterface(Components.interfaces.nsIURL);
            return frameProcessor_callback({title: _url.fileName});
          }
        }
        catch(ex) {
          dump("fail get only title: "+ex+"\n");
        }
      }
	...
	function next(type) {
    _dbg("Next " + type);
    _dbg("Count items in process " + type + " = " + activeCount[type]);
		if( activeCount[type] >= MAX_ACTIVE_GRABBERS ){
			return;
		}

		if( queue[type].length > 0 ){
			var screenData = queue[type].shift();
			process(screenData);
		}
	}
    ...
    var type = screenData.type;
		...
		function fail( resultData ){
          ...
          dump("Error while calling fail callback: " + ex + "\n" + ex.stack + "\n");
				}
			}
    ...
    var processor = null;
      ...
      processor = new frameProcessor(screenData, function(resultData) {
        // check if this dial points to an image
        // if so and request IS NOT for the title only then load the image by imageProcessor
        // because it delivers better appearance for images
        if(
          typeof resultData.contentType === "string" &&
          resultData.contentType.indexOf("image/") === 0 &&
          !screenData.onlyTitle
        ) {
          _dbg("is the dial url picture? let's make screenshot of this picture.");
          // url points to image, make screen via image processor
          screenData.type = "image";
          screenData.imageUrl = screenData.url;
          activeCount[type]--;
          next(type);
          return self.shedule(screenData);
        }
			...
			processor = new imageProcessor( screenData, function( resultData ){
				if( resultData.result ){
					success( resultData );
				}
				else{
	...
	this.shedule = function( screenData ){
		queue[screenData.type].push( screenData );
		next(screenData.type);
	};

};

var fvd_speed_dial_ScreenMaker = new _fvd_speed_dial_ScreenMaker();
