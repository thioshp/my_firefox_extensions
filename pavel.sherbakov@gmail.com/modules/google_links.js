...
var fvd_speed_dial_GoogleLinks = {
	instances: [],
...
var _fvd_speed_dial_GoogleLinks = function( gBrowser ){
    ...
    // check for results container of google
    var resultsContainer = document.getElementById("rso");
    var googleSearchBody = document.getElementById("gsr");
    if(!resultsContainer && !googleSearchBody) {
      return;
    }
      ...
      var blocks;
			try{
				blocks = document.querySelectorAll( "#rso .srg .g" );
			}
			catch( ex ) {
        dump("Fail to fetch google blocks: " + ex + "\n");
				return;
			}
          ...
          if(!a) {
            dump("Can't find A link for google\n");
            continue;
          }
          ...
          if(!greenLineDiv) {
            dump("Can't find green line for google\n");
            continue;
          }
