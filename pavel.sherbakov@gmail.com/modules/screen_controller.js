...
_fvd_speed_dial_ScreenController = function(){
	...
	this.refreshMostVisitedPreview = function( uri ){
      ...
      getTitle: true,
			onSuccess: function( result ){
				...
				if( (!data.title || data.hand_changed == 0) && result.title ){
  ...
  this.refreshSpeedDialTitle = function(dialId, params) {
    _dbg("Requested refresh title of " + dialId);
    var dial = storage.getDialById(dialId);
    if(!dial) {
      _dbg("Dial not found " + dialId);
      return;
    }
    if(dial.hand_changed) {
      _dbg("Can't refresh hand changed titles");
      // can't refresh hand changed titles
      return;
    }
    if(dial.thumb_source_type === "url") {
      _dbg("Title should be obtained from page");
      // title obtained with page
      return;
    }
    storage.updateDialData(dialId, {
      title: "",
      title_status: "loading"
    });
    shedule({
      // add title to parallel get screen and title for dial
      dialId: dialId + "_title",
      type: "url",
      url: dial.url,
      noLoadImages: true,
      onlyTitle: true,
      onSuccess: function(result) {
        ...
        observer.notifyObservers(null, "FVD.Toolbar-SD-Dial-Title-Updated", dialId);
      },
      onFail: function(res) {
        storage.updateDialData(dialId, {
          title_status: "failed"
		...
		if( dial ){
			...
			shedule( {
				...
				onSuccess: function( result ){
					...
					if((!dial.hand_changed || !dial.title) && result.title){
						// if dial not have title or title not hand changed - update this
						updateData.title = result.title;
            updateData.title_status = "done";
						if( dial.title != result.title ){
              ...
              if( dial.title != result.title ){
                ...
                fvd_speed_dial_Sync.syncData( "dials", storage.dialGlobalId( dialId ) );
					...
					if( (dial.thumb_source_type == "force_url" || dial.thumb_source_type == "local_file") && dial.thumb_src ){
						if( !dial.thumb_width && !dial.thumb_height ){
							updateData.status = "thumb_failed"; // thumb grab is failed
						}
