...
AutoCompletePlusResult.prototype = {
    ...
    getValueAt: function(index){
      dump("get value at " + index +", " + this._results[index] + "\n");
      return this._results[index];
    },
    
	getLabelAt: function(index) {
	  dump("get label at " + index +", " + this._results[index] + "\n");
	  return this._results[index];
	},
		...
		globalSearch.getUrl( "http://google.com/complete/search?output=toolbar&q="+encodeURIComponent(searchString), function(){
			...
			if (this.readyState == 4) {
				...
				if ((this.status == 200)) {
					...
					var newResult = new AutoCompletePlusResult(searchString, 
					  Ci.nsIAutoCompleteResult.RESULT_SUCCESS, 0, "", data.items, data.items);
